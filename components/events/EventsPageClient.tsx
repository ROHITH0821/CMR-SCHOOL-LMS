...
export function EventsPageClient({ initialData }: { initialData?: Event[] }) {
  const events = initialData && initialData.length > 0 ? initialData : [
    { day: "12", month: "Apr", title: "Science Fair", venue: "Main auditorium", time: "9:00 AM" },
    { day: "02", month: "May", title: "Sports trials", venue: "Grounds", time: "7:00 AM" },
  ];
              ...
              >
                <div className="flex items-start gap-6">
                  <div className="flex flex-col items-center justify-center bg-gray-50 rounded-2xl p-4 min-w-[80px]">
                    <span className="text-[10px] font-black tracking-widest uppercase text-gray-400 mb-1">{e.month}</span>
                    <span className="text-2xl font-bold text-[#0A2463]">{e.day}</span>
                  </div>
                  <div>
                    <h3 className="font-display text-2xl font-bold text-[#0A2463] group-hover:text-[#F5A623] transition-colors">{e.title}</h3>
